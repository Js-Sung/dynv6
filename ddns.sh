...
{
local a
log_i "checking the existing session"
a=$(curl -m 10 --retry 5 -s -H 'authority: dynv6.com' -H 'accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9' -H 'accept-language: zh-CN,zh;q=0.9,en;q=0.8' -H 'sec-ch-ua-mobile: ?0' -H 'sec-ch-ua-platform: "Windows"' -H 'sec-fetch-dest: document' -H 'sec-fetch-mode: navigate' -H 'sec-fetch-site: none' -H 'sec-fetch-user: ?1' -H 'upgrade-insecure-requests: 1' -H 'user-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/102.0.0.0 Safari/537.36' --compressed -b "$COOKIE" -L 'https://dynv6.com/users/sign_in' | tr -d "\r\n")
[[ -n "$DEBUG" ]] && echo "$a" >3.htm
if echo "$a" | grep -iP 'already signed in' >/dev/null
then
	log_i "session is valid"
else
	log_w "session is invalid"
	return 14
fi
zones_id=$(echo "$a" | grep -ioP 'zones\/\d+[^<]+'"$domin" |  grep -oP '(?<=zones\/)\d+' | sed 'q')
if [[ -z "$zones_id" ]]
then
	log_e 'no matched zone name!'
	return 15
fi
return 0
}
...
if [[ -z "$c1" ]]
then
	log_w "no such record name \"$record_name\" found, add one."		# ---> 添加记录(只支持AAAA类型!!!)
	c2=$(curl -m 10 --retry 5 -s -H "Host: dynv6.com" -H "sec-ch-ua: \" Not A;Brand\";v=\"99\", \"Chromium\";v=\"102\"" -H "x-csrf-token: $csrf_token" -H "sec-ch-ua-mobile: ?0" -H "authorization: Bearer $auth_token" -H "content-type: application/json" -H "accept: application/json, text/plain, */*" -H "user-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/102.0.0.0 Safari/537.36" -H "sec-ch-ua-platform: \"Windows\"" -H "origin: https://dynv6.com" -H "sec-fetch-site: same-origin" -H "sec-fetch-mode: cors" -H "sec-fetch-dest: empty" -H "referer: https://dynv6.com/zones/$zones_id/records" -H "accept-language: zh-CN,zh;q=0.9" --data-binary "{\"name\":\""$record_name"\",\"data\":\""$record_ip"\",\"type\":\"AAAA\"}" --compressed "https://dynv6.com/api/v2/zones/$zones_id/records" | tr -d "\r\n")
	[[ -n "$DEBUG" ]] && echo "$c2" >7.htm
	if ! echo "$c2" | grep -i "$record_ip" >/dev/null
	then
		log_e "failed to add the record"
		return 24
	fi
	return 0
fi
...
if [[ "$t" != "0" ]]
then
	touch "$COOKIE" || { log_e "failed to touch cookie file."; return 1; }
	log_in
	t=$?
fi
